...
- Jessie Cheng generated data during her summer studentship in 2019 on four genes (*BRIP1, RAD51C, RAD51D* and *CHEK2*). This sequencing data was performed on the MinION (see table) and the data was stored on Otago's high capcity storage (`GSFL/Jessie/10th run`). There is an issue with Jessie's fastq files generated by Guppy, I suspect there was a interuption during the copy to long-term storage, as only a 10th of the data is in the `pass` directory. **I will repeat the basecalling using the gridION**.
- Hadley Northcott generated data during his PhD studies in 2022. He sequenced four PCR amplicons across *CDH1, CHEK2, PALB2* and *PTEN* genes. Five samples (PatientID: 18987, 17137, 17069, 18987 and 17905) were assessed and barcoded with barcode 91-95 for multiplex sequecning. The data for which is located on Otago's high capcity storage (`mcrg/Hadley/hn_20012022_flttissue`). There is read's mapped to a sixth barcode which was for a seperate project investigating *SULT1A1* copy number.
...
`guppy-basecaller` is installed on the gridION and with the minKNOW software can be called - however this appeared to cause fatal errors (the computer crashes) - this happened independently for Allison and I on two separate projects. 
>My guess is the config files don't match either the flow cell or kit.

The basecaller can be called through the terminal and with finer control to match the config file to the run. 
...
To run the basecaller only a path to fast5 files, an output file and config file are necessary. However, this will run in CPU mode which is significantly slower than GPU mode. There are options to increase the number of threads (`--cpu_threads_per_caller`, `num_callers`) but even maxed out this wouldn't compare in speed. 
```bash
...
```
Hadley's data was called with no issue originally. There is a slight difference with the read quality threshold. Later versions increased the threshold to 9. In addition Hadley ran the demultiplexing step.
> There is some legacy reasons for the difference in `min_qscore` with default minimum increasing in later minKNOW releases. A large portion of the reads (~2.5 gb) were not assigned to a barcode/sample. 
<table>
    <tr>
...
## Mapping
For both datasets the mapping, and the subsequent steps, the [FLAIR (v.1.5.1) pipeline](https://github.com/BrooksLabUCSC/flair) will be used. The `align` step uses `minimap2` with splice aware options - alternatively this could be run directly.
...
```bash
## For multiple barcoded samples - e.g. Hadley's data
path=/WORKSPACE/George/Hadley_nanoporeData/20220120_1103_X5_FAQ10897_08340024/fastq_pass/
outPath=/WORKSPACE/George/CHEK2_RNAisoforms/nanoporeData/reads/concatanted_reads/
cd $outPath/Hadley
...
```
## Correct
To correct slight errors in splice junction, flair uses known anntataions to adjust alignments. The aligned bed files were corrected using the  gencode **version 41** anntations (hg38 build). The script is on [my github (scripts/correct.sh)](https://github.com/wigge206/CHEK2_RNAisoforms/blob/main/scripts/correct.sh).
```bash
...
```
Once bed files were corrected, all samples from Hadley's data were merged to generate a bed file that will be used to make a 'master' annotation file. This step is not needed for Jessie's data as there is only one sample. 

## Collapse
This step of flair uses the annotation generated by correct step to define RNA isoforms. Reads a collapsed based on matching intron-chains. The gencode **version 41** anntations (hg38 build) are also include to identify known isoforms. See  [scripts/collapse.sh](https://github.com/wigge206/CHEK2_RNAisoforms/blob/main/scripts/collapse.sh) for the my code.
```bash
## General flair correct
...
## Quantify
This step uses the high-qualtiy isoforms from the collapse step and counts the number of reads mapped to each isoform. This requires the generation of a manifest file which assigns samples to batches (this information is more useful for diffExp and diffSplice application - not suitable for this data) See [scripts/quantify.sh](https://github.com/wigge206/CHEK2_RNAisoforms/blob/main/scripts/quantify.sh).
```bash
## General flair correct
...
The single LCL Jessie sequenced had 3,973,018 reads that map to 9,997 features (RNA isoforms). The vast majority of reads map to no known genes (2,116,898) - these RNA are mapping to the chromosome just not within gene boundaries.
...
> Note: As the counts require re-mapping there is the chance that reads can be mapped to other known isoforms. This happened for one read in Jessies data.
...
cd /WORKSPACE/George/CHEK2_RNAisoforms/nanoporeData/align
mkdir CHEK2_bam/ 
...
There is a whole-transcriptome project (NZGL00557) that Vanessa Lau did on one LCL samples treated with and without cycloheximide (NMD inhibitor). The analysis was performed across two lanes on HiSeq sample 1 = Treated and sample 2 = untreated. Each sample library was run on each lane and paired end 100 bp sequencing was performed. This setup results in 4 fastq files per samples, each read direction (R1 and R2) for each lane (L003 and L004). 


## Sample QC
FastQC was run over each analysis 

```bash
path=/WORKSPACE/George/CHEK2_RNAisoforms/shortReadData/
mkdir $path/QC
fastqc fastq/* --threads 4 --outdir QC
```
## Trimming
[Trimmomatic (v.40)](http://www.usadellab.org/cms/?page=trimmomatic) is used to trim Illumina adapters and to remove poor quality reads. To trim adapters, I will need to verified the library prep kit. For most samples, there was no overrepresented sequences which may have indicated adapter read-through. 
I will assume the TruSeq2-PE kit was used and run with leading, trailing and min length settings. These are all set to default settings and are designed to remove poor quality reads and trim poor quality bases. 

The below setting will trim read based in order of:
1. Adapter trimming with a maximum of 2 base pair mismatch tolerance
2. Trim any reads at start of the reads with read score less than 3
3. Trim any reads at the end of the read with score less than 3
4. Remove any reads that have become short (less than 36bp)

FastQC can be re-run to compare how well the trimming perfomred. 

```bash
## Need to instal trimmomatic ()
git clone https://github.com/usadellab/Trimmomatic
cd Trimmomatic
ant ## note I had to tweak the build.xml (https://github.com/usadellab/Trimmomatic/issues/24#issue-1147486679)

timmomatic=/WORKSPACE/George/Tools/Trimmomatic/dist/jar/trimmomatic-0.40-rc1.jar
java -jar $trimmomatic PE input1 input2 output1.paired output1.unpaired output2.paired output2.unpaired ILLUMINACLIP:TruSeq3-PE.fa:2:30:10:2:keepBothReads LEADING:3 TRAILING:3 MINLEN:36
```
As each sample was run on two lanes of HiSeq, these can be concatanted together (e.g `cat sample1_L003_R1.paired.fastq.gz sample1_L004_R1.paired.fastq.gz > sample1_L003.4_R1.paired.fastq.gz`)

## STAR-alignment
The first step for STAR is to generate genome indexs.

### Genome generate
```bash
genome_fa=/STORAGE/Genomes/Gencode/hg38/GRCh38.primary_assembly.genome.fa
annotations=/STORAGE/Genomes/Gencode/hg38/gencode.v41.annotation.gtf
path=/WORKSPACE/George/CHEK2_RNAisoforms/shortReadData/NZGL00557/
cd $path
mkdir genomes
genomeDir=/WORKSPACE/George/CHEK2_RNAisoforms/shortReadData/NZGL00557/genomes/

STAR --runMode genomeGenerate --genomeDir $genomeDir --genomeFastaFiles $genome_fa --sjdbGTFfile $annotations
```

### First pass
```bash
## STAR alignment -- first pass
genome_fa=/STORAGE/Genomes/Gencode/hg38/GRCh38.primary_assembly.genome.fa
annotations=/STORAGE/Genomes/Gencode/hg38/gencode.v41.annotation.gtf
genomeDir=/WORKSPACE/George/CHEK2_RNAisoforms/shortReadData/NZGL00557/genomes/
path=/WORKSPACE/George/CHEK2_RNAisoforms/shortReadData/NZGL00557/
...
do
  ...
  out=NZGL00557_sample$i
  STAR --genomeDir $genomeDir --readFilesIn $file $file2 --readFilesCommand zcat --alignIntronMax 50000 --outFileNamePrefix $out
done
cd ..
```
